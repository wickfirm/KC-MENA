...
export default function HomeEditor({ content, status, seo }: Props) {
  ...
  return <form onSubmit={save} className="card" style={{ maxWidth: 960 }}>
    ...
    <section style={sectionStyle}><h2 style={{ fontSize: "1.2rem", marginBottom: 16 }}>Hero</h2><Field name="hero-heading" label="Heading" value={hero.heading} /><Field name="hero-body" label="Introduction" value={hero.body} multiline /><Field name="hero-video" label="Hero video URL" value={hero.videoSrc} /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name="hero-poster" label="Poster image URL" value={hero.image?.src} /><Field name="hero-poster-alt" label="Poster image alt text" value={hero.image?.alt} /></div></section>
    ...
    <section style={sectionStyle}><h2 style={{ fontSize: "1.2rem", marginBottom: 16 }}>Local businesses</h2><Field name="local-heading" label="Heading" value={local.heading} /><Field name="local-body" label="Introduction" value={local.body} multiline />{local.items.map((item, index) => <div key={index} style={sectionStyle}><h3 style={{ fontSize: "1rem", marginBottom: 14 }}>Card {index + 1}</h3><Field name={`local-title-${index}`} label="Title" value={item.title} /><Field name={`local-body-${index}`} label="Description" value={item.body} multiline /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name={`local-image-${index}`} label="Image URL" value={item.image?.src} /><Field name={`local-image-alt-${index}`} label="Image alt text" value={item.image?.alt} /></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name={`local-link-label-${index}`} label="Link label" value={item.link?.label} /><Field name={`local-link-${index}`} label="Link URL" value={item.link?.href} /></div></div>)}</section>
    <section style={sectionStyle}><h2 style={{ fontSize: "1.2rem", marginBottom: 16 }}>Locations</h2><Field name="locations-heading" label="Heading" value={locations.heading} /><Field name="locations-body" label="Introduction" value={locations.body} multiline />{locations.locations.map((item, index) => <div key={index} style={sectionStyle}><h3 style={{ fontSize: "1rem", marginBottom: 14 }}>Location {index + 1}</h3><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name={`location-label-${index}`} label="Label" value={item.label} /><Field name={`location-city-${index}`} label="City" value={item.city} /></div><Field name={`location-address-${index}`} label="Address" value={item.address} multiline /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name={`location-image-${index}`} label="Image URL" value={item.image.src} /><Field name={`location-image-alt-${index}`} label="Image alt text" value={item.image.alt} /></div><Field name={`location-map-${index}`} label="Map URL" value={item.mapLink?.href} /></div>)}</section>
    <section style={sectionStyle}><h2 style={{ fontSize: "1.2rem", marginBottom: 16 }}>Global businesses</h2><Field name="global-heading" label="Heading" value={global.heading} />{global.items.map((item, index) => <div key={index} style={sectionStyle}><h3 style={{ fontSize: "1rem", marginBottom: 14 }}>Card {index + 1}</h3><Field name={`global-title-${index}`} label="Title" value={item.title} /><Field name={`global-body-${index}`} label="Description" value={item.body} multiline /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}><Field name={`global-image-${index}`} label="Image URL" value={item.image?.src} /><Field name={`global-image-alt-${index}`} label="Image alt text" value={item.image?.alt} /></div><Field name={`global-tags-${index}`} label="Tags (comma-separated)" value={item.tags?.join(", ")} /></div>)}</section>
